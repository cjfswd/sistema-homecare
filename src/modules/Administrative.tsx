...
import type { Patient, Professional, Service, RecordStatus, Evolution } from '@/types';
import { MOCK_PATIENTS, MOCK_PROFESSIONALS, MOCK_SERVICES, INITIAL_EVOLUTIONS } from '@/lib/mockData';
import { Button, Modal, StatusBadge, Pagination, usePagination, type Tab } from '@/components/ui';
import { SearchInput } from '@/components/forms';
import { PatientForm, ProfessionalForm, ServiceForm, ClinicalHistoryModal } from '@/components/administrative';
import { roleLabels } from '@/lib/translations';
import { loggingService } from '@/lib/loggingService';

type ActiveTab = 'patients' | 'professionals' | 'services';

export default function AdministrativeModule() {
  ...
  const openModal = (data?: any) => {
    if (data) {
      ...
      if (activeTab === 'services') setFormData({ active: true, category: 'procedure' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (activeTab === 'patients') {
      ...
      loggingService.log({
        ...
        description: `${editingId ? 'Atualizou' : 'Criou'} serviço: ${newService.name}`
      });
    }
    closeModal();
  };
  ...
  const handleDelete = (id: string) => {
    ...
    if (activeTab === 'patients') {
      ...
      description = `Excluiu serviço: ${s?.name}`;
      entity = 'Service';
  ...
  const getModalTitle = () => {
    ...
    const entity = activeTab === 'patients' ? 'Paciente' : activeTab === 'professionals' ? 'Profissional' : 'Serviço';
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans text-slate-800 pb-10">
      <header className="bg-white border-b border-slate-200 px-8 py-6 sticky top-0 z-10">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Cadastros base, gestão de pessoas e catálogo de serviços.</p>
          </div>
          <Button onClick={() => openModal()} icon={Plus}>
            {`Novo ${activeTab === 'patients' ? 'Paciente' : activeTab === 'professionals' ? 'Profissional' : 'Serviço'}`}
          </Button>
        </div>
      ...
      <main className="p-8 max-w-7xl mx-auto">
        <div className="mb-6">
          <SearchInput 
            ...
            placeholder={
              activeTab === 'patients' ? "Buscar por nome ou CPF..." : 
              activeTab === 'professionals' ? "Buscar por nome ou conselho..." : "Buscar serviço..."
            }
          />
        </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  <tr key={service.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button variant="ghost" size="sm" onClick={() => openModal(service)}>Editar</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      >
        ...
        {activeTab === 'services' && <ServiceForm formData={formData} onFormChange={handleFormChange} />}
      </Modal>
